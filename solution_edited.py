...
def get_prompt_lengths(tokenizer, prompts, max_length):
    """Return tokenized prompt lengths so aggregation can isolate response tokens."""
    lengths = []
...
if __name__=='__main__':
    ...
    all_prompts = df["prompt"].astype(str).tolist()
    all_prompt_lengths = get_prompt_lengths(tokenizer, all_prompts, MAX_LENGTH)
    ...
    test_prompts = df_test["prompt"].astype(str).tolist()
    test_prompt_lengths = get_prompt_lengths(tokenizer, test_prompts, MAX_LENGTH)
    ...
    for start in tqdm(range(0, len(test_texts), BATCH_SIZE),
        ...
        batch_prompt_lengths = test_prompt_lengths[start : start + BATCH_SIZE]
        encoding = tokenizer(
            batch_texts,
            return_tensors="pt",
            padding=True,

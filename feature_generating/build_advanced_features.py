"""
Build advanced features for hallucination detection.

This script intentionally requires NO changes to solution.py:
- no prompt_len
- no output_attentions
- no logits
- no hooks

It creates one additional parquet family with features:
- max/mean pooling on mid layers
- EOS stripping via dropping the last active token
- layer-16 additions
- weighted top-3 last-token aggregation
- compact last-token geometry
- compact L10-L19 geometry
- SGI-like prompt/response proxy geometry
- cross-layer update norms
- response-proxy temporal dynamics
- ICR-lite spectral consistency features
- prompt/response centroid proxy features
- truncation/fallback flags

Outputs:
./artifacts/advanced_features/features_dataset_advanced.parquet
./artifacts/advanced_features/features_test_advanced.parquet
"""
...
OUTPUT_DIR = Path("./artifacts/advanced_features")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TRAIN_OUTPUT = OUTPUT_DIR / "features_dataset_advanced.parquet"
TEST_OUTPUT = OUTPUT_DIR / "features_test_advanced.parquet"
...
def extract_dataset_features(
    ...
    for start in tqdm(range(0, len(texts), BATCH_SIZE), desc="Extract advanced features"):
        ...
        for i in range(hidden_batch.shape[0]):
            rows.append(extract_features_one_sample(hidden_batch[i], mask_batch[i]))
    ...
    # Keep text columns for later joins/debugging, matching existing builders.
...
def main() -> None:
    ...
    print("=" * 80)
    print("BUILD ADVANCED COMPETITOR-INSPIRED FEATURES")
    print("=" * 80)
